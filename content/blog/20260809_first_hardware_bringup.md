...
That's a real double standard, and I want to be honest about why I made it. DDR3L timing and MIO/clock parameters are genuinely risky to get wrong by hand — get them slightly off and you don't get a clean failure, you get a board that's flaky in ways that are brutal to diagnose, because you can no longer trust the platform underneath whatever bug you're chasing. Everything else in this project is worth learning by re-deriving it myself. The PS7 configuration is not one of those things; it's exactly the kind of parameter set a vendor has already validated in silicon, and re-deriving it from scratch would have bought me risk without buying me understanding.

First real task on the inherited design was mechanical: upgrading it from Vivado 2021 to 2025, which meant regenerating the PS7 IP cell to the newer revision. In the middle of that work I found a duplicate `design_1_processing_system7_0_0 copy.xci` file sitting in the repo — the kind of thing that looks like accidental clutter. It wasn't; it was a deliberate backup made before the upgrade, kept on purpose until a real cleanup pass later. Small moment, but it set a rule that held for the rest of the project: don't assume something untidy-looking is safe to delete. Ask first.

## The AXI bug that simulation could never have caught

The first real hardware milestone broke immediately. I'd wired a hand-written `axi_interconnect` — a simple 1-master/N-slave AXI address decoder — between the PS7's `M_AXI_GP0` port and the peripheral cluster. The moment I loaded the bitstream, the entire UART register console went silent. Two independent bugs, stacked on top of each other.
...
Both bugs turned into a permanent section of the project README: a "Zynq-7000 GP0/GP1 address map — read before picking any AXI base address" writeup. The generalized lesson stuck with me for the rest of the bring-up: anything that depends on real AXI master behavior, or on the PS/PL address split, is invisible until you're on actual hardware, no matter how clean your simulation looks.
...
With SPI alive and the register console talking to real silicon, the project had its actual starting line. What came next was a bug that took a lot longer to run down — a hang that would only show up after the board had been running for a while, and that took ruling out most of the ARM core's own errata before the real cause turned up somewhere nobody was looking. That's next.
